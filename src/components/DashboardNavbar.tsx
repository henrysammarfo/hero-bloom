          ...
          {isConnected && address ? (
            <>
              <div className="flex items-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-2 rounded-xl bg-secondary/50 border border-border/20">
                ...
                <span className="text-xs font-mono text-foreground hidden sm:inline">{shortAddress}</span>
                <span className="text-xs font-mono text-foreground sm:hidden">{address.slice(0, 4)}…{address.slice(-2)}</span>
                <button onClick={handleCopy} className="p-1 rounded-md hover:bg-secondary transition-colors" aria-label="Copy address">
