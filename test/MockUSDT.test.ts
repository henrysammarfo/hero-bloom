...
describe("MockUSDT", function () {
  ...
  it("Should reject mint to zero address", async function () {
    ...
    const token = await MockUSDT.deploy();
    await token.waitForDeployment();
    await expect(token.mint(ethers.ZeroAddress, ethers.parseUnits("1", 6))).to.be.revertedWith("MockUSDT: mint to zero");
  });
  ...
  it("Should allow faucet after cooldown", async function () {
    ...
    expect(await token.balanceOf(user.address)).to.equal(balBefore + (await token.FAUCET_AMOUNT()));
  });
  ...
  it("Should reject faucet during cooldown", async function () {
    ...
    const token = await MockUSDT.deploy();
    await token.waitForDeployment();
    await token.mint(await token.getAddress(), ethers.parseUnits("100000", 6));
    await token.connect(user).requestFaucet();
    await expect(token.connect(user).requestFaucet()).to.be.revertedWith("MockUSDT: cooldown");
  });
  ...
  it("Should reject faucet when reserve empty", async function () {
    ...
    const token = await MockUSDT.deploy();
    await token.waitForDeployment();
    await expect(token.connect(user).requestFaucet()).to.be.revertedWith("MockUSDT: faucet empty");
  });
});
